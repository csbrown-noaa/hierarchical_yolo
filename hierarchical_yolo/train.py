...
def train_curriculum(
    ...
    ----------
    workspace_dir : str
        The root directory of the compiled hierarchical workspace.
    model_dir : str
    ...
    if not os.path.exists(hierarchy_json_path):
        raise FileNotFoundError(f"Hierarchy JSON not found: {hierarchy_json_path}. Did you run the data orchestrator?")
    ...
    for depth in range(max_depth + 1):
        ...
        data_yaml = os.path.join(workspace_dir, "tier_yolo_full_head", f"{depth:03d}", "train.yaml")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str, 
        required=True,
        help="Path to the compiled workspace directory (e.g. generated by data_orchestrator.py)."
    )
    parser.add_argument(
        '--model_dir', 
        type=str, 
