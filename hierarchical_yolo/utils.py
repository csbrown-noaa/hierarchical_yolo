...
def hierarchical_loss2(pred, targets, hierarchy_index):
    ...
    ultralytics.utils.LOGGER.info(hierarchical_summed_log1sigmoids)
    return -(
...
def postprocess_raw_output(raw_yolo_output, hierarchy):
    all_boxes = []
    all_class_scores = []
    _, nms_idxs = ultralytics.utils.ops.non_max_suppression(raw_yolo_output, classes=get_roots(hierarchy), return_idxs=True, iou_thres=0.7, conf_thres=0.25)
